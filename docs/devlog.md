...
背景：
- 学者页“本地 PDF 索引”的“当前索引条目”容易被理解成项目里所有 PDF 的总数。
- 实际上该数字只表示 `ACADEMIC_IMPACT_PDF_LIBRARY_DIRS` / `ACADEMIC_IMPACT_DOWNLOAD_DIR` 配置扫描目录中被索引到的 PDF，不包含当前队列手动上传但未进入扫描目录的 PDF。
- 学者页此前没有和单篇论文页一致的“展开 / 收起”模块按钮，长页面浏览成本偏高。
...
- 学者页本地 PDF 模块改名展示为：
  ...
  - 已上传/已绑定 PDF
  - 队列可直接分析 PDF
- 本地 PDF 模块新增说明：“索引数字只统计下面扫描目录中的 PDF；上传到当前队列的 PDF 会单独计入已上传/已绑定 PDF。”
- 本地 PDF 状态新增扫描目录存在性统计，显示配置目录中实际存在的目录数量，帮助排查“索引数量太少”是否由目录配置导致。
- 学者页所有带 `.panel-header` 的主模块和子模块会自动获得“展开 / 收起”按钮，并用浏览器本地存储记住折叠状态。
- 修复人物标签候选区折叠不完整的问题：候选区内容统一放入 `.panel-body`，并为折叠态补充 CSS 兜底，确保模块头部之外的直接内容都会被隐藏。

当前策略：
- 本地 PDF 索引仍是轻量 JSON 索引，不引入数据库。
- 刷新本地 PDF 索引只刷新 PDF 相关状态和当前队列重匹配，不重建人物候选、venue 统计或高价值队列。
- 如果索引数量看起来偏少，优先检查页面显示的扫描目录是否指向真实论文库目录。
...
本次处理：
- 模板匹配结果会显示具体命中的模板标签/关键词，例如 `模板命中词：最先进 / SOTA / state-of-the-art`。
- 未命中时也显示当前模板正在检查的前几个关键词，例如 `当前模板检查：首次 / 开创性评价 / first / first work`。
- 分析结果卡片摘要层也显示“模板命中词 / 模板检查词”，不再只藏在“结构化判断明细”里。
- 命中模板和未命中模板使用不同颜色的 chip，便于和普通标签区分。
- 模块折叠改为固定 `.panel-body` 容器，并由 JS 直接设置正文的 `hidden` 状态，同时继续保留本地记忆。
- `localStorage` 不可用时也能正常折叠，只是不保存状态。

当前策略：
- 弱提及、组引用和 `keep=false` 仍不会算作模板命中，即使它们带有模板标签或关键词。
- “当前模板检查”展示的是当前模板的检查范围，不代表模型没有看到这些词，而是该 finding 没有满足模板命中条件。
