...
class PersonRegistryRefreshTestCase(unittest.TestCase):
    ...
    def test_refresh_preserves_tags_array_seed_entries(self):
        with tempfile.TemporaryDirectory() as tmpdir:
                        ...
                        'items': [
                            {
                                'name': 'Andrew Yao',
                                'aliases': ['姚期智'],
                                'matched_affiliations': ['清华大学'],
                                'tags': [
                                    {
                                        'type': 'acm_fellow',
                                        'source_links': ['https://example.com/acm-old'],
                                    },
                                    {
                                        'type': 'godel_prize',
                                        'source_links': ['https://example.com/godel-old'],
                                    },
                                ],
                            }
                        ]
            ...
            (source_dir / 'fellows.csv').write_text(
                'name,tag_type,aliases,source_links,matched_affiliations,note\n'
                'Andrew Yao,acm_fellow,Andrew Chi-Chih Yao,https://example.com/acm-new,Tsinghua University,official source\n',
                encoding='utf-8',
            )
            ...
            payload = json.loads(registry_path.read_text(encoding='utf-8'))
            items = {(item['tag_type'], item['name']): item for item in payload['items']}
            self.assertIn(('acm_fellow', 'Andrew Yao'), items)
            self.assertIn(('godel_prize', 'Andrew Yao'), items)
            self.assertIn('姚期智', items[('acm_fellow', 'Andrew Yao')]['aliases'])
            self.assertIn('Andrew Chi-Chih Yao', items[('acm_fellow', 'Andrew Yao')]['aliases'])
            self.assertIn('https://example.com/acm-old', items[('acm_fellow', 'Andrew Yao')]['source_links'])
            self.assertIn('https://example.com/acm-new', items[('acm_fellow', 'Andrew Yao')]['source_links'])
            self.assertIn('https://example.com/godel-old', items[('godel_prize', 'Andrew Yao')]['source_links'])
