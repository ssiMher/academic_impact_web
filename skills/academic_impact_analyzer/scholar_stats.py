...
def load_venue_registry(path: str | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return []
    items = payload if isinstance(payload, list) else payload.get("items") or payload.get("entries", [])
    ...
    for item in items:
        ...
        tier = normalize_tier_label(item)
        if not name:
            continue
        ...
        entries.append(
            {
                "name": name,
                "tier": tier,
                "category": (item.get("category") or item.get("venue_type") or "").strip(),
...
def normalize_tier_label(item: dict[str, Any]) -> str:
    tier = str(item.get("tier") or "").strip()
    tier_system = str(item.get("tier_system") or "").strip().upper()
    tier_label = str(item.get("tier_label") or "").strip()
    if tier_system == "CCF" and tier:
        return f"CCF-{tier.upper()}"
    if tier.lower() == "top" or tier_label.lower() == "top venue seed":
        return "Top"
    if tier.upper() in {"CCF-A", "CCF-B", "CCF-C"}:
        return tier.upper()
    return tier or "Unmatched"
